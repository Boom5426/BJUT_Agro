...
def mask_tensor_with_masks(X, mask_zero_ratio, mask_nonzero_ratio, device='cuda:0'):
    X = X.to(device)
    nonzero_indices = torch.nonzero(X, as_tuple=True) #　获取张量 X 中所有非零元素的索引。
    num_nonzero_to_mask = int(round(mask_nonzero_ratio * len(nonzero_indices[0]))) # 计算需要掩码的非零元素的数量。
    nonzero_mask_indices = torch.randperm(len(nonzero_indices[0]))[:num_nonzero_to_mask] #　随机选择需要掩码的非零元素

    zero_indices = torch.nonzero(X == 0, as_tuple=True) # 获取零元素的索引
    num_zero_to_mask = int(round(mask_zero_ratio * len(zero_indices[0]))) #　计算需要掩码的零元素数量
    zero_mask_indices = torch.randperm(len(zero_indices[0]))[:num_zero_to_mask]  # 随机选择需要掩码的零元素

    masked_X = X.clone()
    masked_X[nonzero_indices[0][nonzero_mask_indices], nonzero_indices[1][nonzero_mask_indices]] = 0 
    masked_X[zero_indices[0][zero_mask_indices], zero_indices[1][zero_mask_indices]] = 0  #　掩码后的张量

    M_nonzero_mask = torch.zeros_like(X, dtype=torch.float32)
    M_nonzero_mask[nonzero_indices[0][nonzero_mask_indices], nonzero_indices[1][nonzero_mask_indices]] = 1.0  #　生成非零元素的掩码矩阵

    M_zero_mask = torch.zeros_like(X, dtype=torch.float32)
    M_zero_mask[zero_indices[0][zero_mask_indices], zero_indices[1][zero_mask_indices]] = 1.0  # 生成零元素的掩码矩阵
...
def split_dataset_with_smiles_names(dataset, train_ratio=0.8, random_state=None):

    total_size = len(dataset)
    indices = list(range(total_size))
    print('Total_size:', total_size) # 20346
    ...
    #     raise ValueError("训练集和测试集的大小之和超过了数据集的总大小。")
    
    # # 生成训练集和测试集的索引
    # train_indices = list(range(train_size))  # 0 到 25364
    # test_indices = list(range(train_size, train_size + test_size))  # 25364 到 25378
    ...
    train_indices, test_indices = train_test_split(indices, train_size=train_ratio, random_state=random_state)

    train_dataset = torch.utils.data.Subset(dataset, train_indices)
    test_dataset = torch.utils.data.Subset(dataset, test_indices)

    train_gene_names = [dataset.get_smiles_names()[i] for i in train_indices]
    test_gene_names = [dataset.get_smiles_names()[i] for i in test_indices]

    return (train_dataset, train_gene_names), (test_dataset, test_gene_names)
